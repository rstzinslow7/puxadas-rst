...
║   Sistema de consulta institucional     ║
║   dc: ristoteles7                       ║
╚══════════════════════════════════════════╝
...
DEFAULT_ADMIN_USER = "rst7"
DEFAULT_ADMIN_PASS = "@2026"
...
FUNC_PERMISSIONS = {
    "consulta_pessoa":   [Role.OPERADOR, Role.AUDITOR, Role.ADMIN],
...
def login(username: str, password: str, session: Session) -> tuple:
    ...
    if username not in users:
        _log_auth("LOGIN_FAIL", username, "usuário não existe")
        return False, "Usuário ou senha inválidos.", session

    user = users[username]

    if not user.get("active", True):
        _log_auth("LOGIN_FAIL", username, "conta desativada")
        return False, "Conta desativada.", session

    if user.get("locked_until", 0) > time.time():
        remaining = int(user["locked_until"] - time.time())
        _log_auth("LOGIN_LOCKED", username, f"bloqueado por {remaining}s")
        return False, f"Conta bloqueada. Tente em {remaining}s.", session

    if not verify_password(password, user["password_hash"]):
        user["login_attempts"] = user.get("login_attempts", 0) + 1
        if user["login_attempts"] >= MAX_LOGIN_ATTEMPTS:
            user["locked_until"] = time.time() + LOCKOUT_DURATION
            _log_auth("LOGIN_LOCKOUT", username, f"{MAX_LOGIN_ATTEMPTS} tentativas falhas")
            _save_users(users)
            return False, f"Conta bloqueada por {LOCKOUT_DURATION}s.", session
        _save_users(users)
    ...
    _log_auth("LOGIN_OK", username, "", success=True)
    return True, f"Bem-vindo, {username}!", session
...
def menu():
    ...
    if session.is_valid() and has_permission(session.role, "auditoria"):
        print(f"{CYAN}[6]{RESET} Auditoria")

    if session.is_valid() and has_permission(session.role, "gerenciar_usuarios"):
        print(f"{CYAN}[7]{RESET} Gerenciar Usuários")
